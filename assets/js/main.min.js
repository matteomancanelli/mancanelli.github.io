...
-$title.outerWidth(true)-($lang_switch.length!==0?$lang_switch.outerWidth(true):0)
-($search.length!==0?$search.outerWidth(true):0)
-(numOfVisibleItems!==breakWidths.length?$btn.outerWidth(true):0);requiredSpace=breakWidths[numOfVisibleItems-1];if(requiredSpace>availableSpace){$vlinks.children().last().prependTo($hlinks);numOfVisibleItems-=1;check();return;}else if(availableSpace+(numOfVisibleItems===breakWidths.length-1?$btn.outerWidth(true):0)>breakWidths[numOfVisibleItems]){$hlinks.children().first().appendTo($vlinks);numOfVisibleItems+=1;check();return;}
if(numOfVisibleItems!==numOfItems&&numOfVisibleItems>0){$($vlinks.children().get().reverse()).each(function(){$(this).prependTo($hlinks);});numOfVisibleItems=0;}
...
$(window).resize(function(){check();});function remeasure(){measureLinks();check();}
$(window).on('load',remeasure);if(document.fonts&&document.fonts.ready){document.fonts.ready.then(remeasure);}
$btn.on('click',function(e){e.stopPropagation();$hlinks.toggleClass('hidden');$(this).toggleClass('close');clearTimeout(timer);});$hlinks.on('mouseleave',function(){timer=setTimeout(function(){$hlinks.addClass('hidden');$btn.removeClass('close');},closingTime);}).on('mouseenter',function(){clearTimeout(timer);});$(document).on('click touchend',function(e){if(!$(e.target).closest('.greedy-nav__toggle, .hidden-links').length){$hlinks.addClass('hidden');$btn.removeClass('close');clearTimeout(timer);}});if($logoImg.length!==0){if(!($logoImg[0].complete||$logoImg[0].naturalWidth!==0)){$logoImg.one("load error",check);}else check();}else check();});;
...
$(function(){$("#main").fitVids();var stickySideBar=function(){var show=$(".author__urls-wrapper").find("button").length===0?$(window).width()>1024:!$(".author__urls-wrapper").find("button").is(":visible");if(show){$(".sidebar").addClass("sticky");}else{$(".sidebar").removeClass("sticky");}};stickySideBar();$(window).resize(function(){stickySideBar();});$(".author__urls-wrapper").find("button").on("click",function(){$(".author__urls").toggleClass("is--visible");$(".author__urls-wrapper").find("button").toggleClass("open");});$(document).keyup(function(e){if(e.keyCode===27){if($(".initial-content").hasClass("is--hidden")){$(".search-content").toggleClass("is--visible");$(".initial-content").toggleClass("is--hidden");}}});$(".search__toggle").on("click",function(){$(".search-content").toggleClass("is--visible");$(".initial-content").toggleClass("is--hidden");setTimeout(function(){$(".search-content").find("input").focus();},400);});var scroll=new SmoothScroll('a[href*="#"]',{offset:20,speed:400,speedAsDuration:true,durationMax:500});if($("nav.toc").length>0){var spy=new Gumshoe("nav.toc a",{navClass:"active",contentClass:"active",nested:false,nestedClass:"active",offset:20,reflow:true,events:true});}
$("a[href$='.jpg'],a[href$='.jpeg'],a[href$='.JPG'],a[href$='.png'],a[href$='.gif'],a[href$='.webp']").has("> img").addClass("image-popup");$(".image-popup").magnificPopup({type:"image",tLoading:"Loading image #%curr%...",gallery:{enabled:true,navigateByImgClick:true,preload:[0,1]},image:{tError:'<a href="%url%">Image #%curr%</a> could not be loaded.'},removalDelay:500,mainClass:"mfp-zoom-in",callbacks:{beforeOpen:function(){this.st.image.markup=this.st.image.markup.replace("mfp-figure","mfp-figure mfp-with-anim");}},closeOnContentClick:true,midClick:true});$('.page__content').find('h1, h2, h3, h4, h5, h6').each(function(){var id=$(this).attr('id');if(id){var anchor=document.createElement("a");anchor.className='header-link';anchor.href='#'+id;anchor.innerHTML='<span class=\"sr-only\">Permalink</span><i class=\"fas fa-link\"></i>';anchor.title="Permalink";$(this).append(anchor);}});});
